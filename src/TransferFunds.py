            ...
            for row in results:
                ...
                if last_active_date_str:
                    ...
                    if last_active_date + timedelta(days=365) < current_date:
                        ...
                        # Transfer funds to the heir's balance if heir is not None and balance is not None
                        if heir is not None and balance is not None:
                            ...
                            # Transfer cryptocurrency ownership to the heir
                            select_crypto_query = f"SELECT `Crypto Name`, `Crypto Amount` FROM crypto WHERE Username = '{username}'"
                            cursor.execute(select_crypto_query)
                            crypto_results = cursor.fetchall()

                            for crypto_row in crypto_results:
                                crypto_name = crypto_row[0]
                                crypto_amount = crypto_row[1]

                                # Update the owner of the cryptocurrency to the heir
                                transfer_crypto_query = f"UPDATE crypto SET Username = '{heir}' WHERE Username = '{username}' AND `Crypto Name` = '{crypto_name}'"
                                cursor.execute(transfer_crypto_query)
                                print(f"Transferred {crypto_amount} {crypto_name} ownership from {username} to {heir}")
